...
BEGIN
    ...
    -- 0 is also a valid end state: a from-scratch database (built from
    -- genesis_foundational_tables.sql, itself a dump of prod's current
    -- *public* schema) never had any cache_server* tables in public to
    -- begin with -- prod had already archived them into
    -- archive_cache_servers, a schema genesis deliberately never captures.
    -- 6 is the real-prod-migration outcome; 0 is the fresh-install outcome.
    IF archived_count NOT IN (0, 6) THEN
        RAISE EXCEPTION 'expected 0 or 6 archived cache_server tables, found %', archived_count;
    END IF;
...
-- +goose Down
-- One-way historical port from exp/rewrite/remove_cache_servers.sql -- no rollback written;
